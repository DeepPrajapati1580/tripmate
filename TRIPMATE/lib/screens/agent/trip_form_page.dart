      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 12),
            TextFormField(controller: _sourceCtrl, decoration: _inputDecoration("Source City", icon: Icons.location_city), validator: (v) => v!.isEmpty ? "Enter source city" : null),
            const SizedBox(height: 12),
            TextFormField(controller: _destCtrl, decoration: _inputDecoration("Destination", icon: Icons.place), validator: (v) => v!.isEmpty ? "Enter destination" : null),
            const SizedBox(height: 12),
            TextFormField(controller: _priceCtrl, decoration: _inputDecoration("Price (₹)", icon: Icons.currency_rupee), keyboardType: TextInputType.number, validator: (v) => v!.isEmpty ? "Enter price" : null),
            const SizedBox(height: 12),
            TextFormField(controller: _capacityCtrl, decoration: _inputDecoration("Capacity", icon: Icons.people), keyboardType: TextInputType.number, validator: (v) => v!.isEmpty ? "Enter capacity" : null),
            ...
            const SizedBox(height: 12),
            TextFormField(controller: _mealsCtrl, decoration: _inputDecoration("Meals (comma separated)", icon: Icons.restaurant)),
            const SizedBox(height: 12),
            TextFormField(controller: _activitiesCtrl, decoration: _inputDecoration("Activities (comma separated)", icon: Icons.sports)),
            const SizedBox(height: 12),
            SwitchListTile(value: _airportPickup, onChanged: (v) => setState(() => _airportPickup = v), title: const Text("Airport Pickup Available")),
            const SizedBox(height: 16),
            // Dates
            Row(children: [
              Expanded(child: OutlinedButton.icon(onPressed: _pickStartDate, icon: const Icon(Icons.calendar_today), label: Text(_startDate == null ? "Pick Start Date" : "Start: ${_startDate!.day}/${_startDate!.month}/${_startDate!.year}"))),
              const SizedBox(width: 12),
              Expanded(child: OutlinedButton.icon(onPressed: _pickEndDate, icon: const Icon(Icons.event), label: Text(_endDate == null ? "Pick End Date" : "End: ${_endDate!.day}/${_endDate!.month}/${_endDate!.year}"))),
            ]),
            const SizedBox(height: 16),
            // Cover image
            GestureDetector(
              onTap: () => _pickImage((f) => setState(() => _coverImage = f)),
              child: Container(
                height: 150,
                ...
                child: _coverImage != null ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(_coverImage!, fit: BoxFit.cover, width: double.infinity)) : const Center(child: Text("Tap to pick Cover Image", style: TextStyle(color: Colors.grey))),
              ),
            ),
            const SizedBox(height: 12),
            OutlinedButton.icon(onPressed: () => _pickMultipleImages((list) => setState(() => _galleryImages = list)), icon: const Icon(Icons.photo_library), label: const Text("Pick Gallery Images")),
            Wrap(spacing: 8, runSpacing: 8, children: _galleryImages.map((f) => Image.file(f, width: 80, height: 80, fit: BoxFit.cover)).toList()),
            const SizedBox(height: 12),
            // Hotel images
            OutlinedButton.icon(onPressed: () => _pickImage((f) => setState(() => _hotelMainImage = f)), icon: const Icon(Icons.hotel), label: const Text("Pick Hotel Main Image")),
            const SizedBox(height: 8),
            OutlinedButton.icon(onPressed: () => _pickMultipleImages((list) => setState(() => _hotelGalleryImages = list)), icon: const Icon(Icons.photo_library), label: const Text("Pick Hotel Gallery Images")),
            Wrap(spacing: 8, runSpacing: 8, children: _hotelGalleryImages.map((f) => Image.file(f, width: 80, height: 80, fit: BoxFit.cover)).toList()),
            const SizedBox(height: 16),
            // Itinerary
            ...
            Column(children: _itineraryControllers.map((c) => ItineraryDayField(
              dayCtrl: c["day"]!,
              descCtrl: c["desc"]!,
              mealsCtrl: c["meals"]!,
